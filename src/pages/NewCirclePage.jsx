...
// Members may be bare ID strings or fully-hydrated objects {id, name, icon, ...}.
// Full objects are sent as-is to the backend (no redundant lookup).

import { useState, useRef } from 'react'
import { useNavigate, useLocation, Link } from 'react-router-dom'
import { useTranslation } from 'react-i18next'
import { Loader, UserPlus, UserMinus, X } from 'lucide-react'
...
// ── MemberChip — member in the initial list ───────────────────────────────────
...
// ── AddMemberRow — look up and stage a new member ────────────────────────────
...
export default function NewCirclePage() {
  ...
  const [members, setMembers] = useState(() =>
    (initial.members ?? []).map(normalizeMember)
  )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let iconUrl = iconFile ? null : (iconPreview ?? null)
      if (iconFile) {
        ...
        if (uploaded?.file?.url) iconUrl = uploaded.file.url
      }
      ...
      // 3. Batch-add initial members (full objects preserved — no redundant lookups)
      if (members.length > 0) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-8 pb-12">
      ...
      <div className="flex flex-col gap-4">
        ...
        <AddMemberRow onAdd={handleAddMember} existingIds={existingIds} />

        {members.length > 0 && (
          <div className="flex flex-col gap-2 mt-1">
            {members.map((member) => (
              <MemberChip key={member.id} member={member} onRemove={handleRemoveMember} />
            ))}
          </div>
        )}
      </div>
